...
if __name__ == '__main__':
    ...
    train_dataset = CityscapeDataset(csv_file='/mnt/nvme/work/phone_labeling/train.csv',
                                     root_dir='/mnt/nvme/work/phone_labeling/img_raw')

    valid_dataset = CityscapeDataset(csv_file='/mnt/nvme/work/phone_labeling/train.csv',
                                     root_dir='/mnt/nvme/work/phone_labeling/img_raw')

    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=2, shuffle=True, num_workers=1,collate_fn=collate_func)
    valid_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=2, shuffle=True, num_workers=1,
                                               collate_fn=collate_func)

    net = Network()
    net = torch.load('/mnt/nvme/adas/code/phone_labeling/networks/checkpoint__net356.pt')
    ...
    #optimizer = optim.SGD(net.parameters(), lr=1.0e-7, weight_decay=0.0005, momentum=0.99)
    '''
